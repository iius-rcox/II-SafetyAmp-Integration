...
class ErrorNotifier:
    ...
    def log_error(
        ...
        try:
            _errors_counter.labels(
                error_type=error_type, entity_type=entity_type, source=source
            ).inc()
        except Exception:
            pass
    ...
    def get_errors_since(self, hours: int = 1) -> List[Dict[str, Any]]:
        ...
        return [
            error for error in self.errors if error["timestamp"] >= cutoff_timestamp
        ]
        ...
        if summary["errors"]:
            ...
            for i, error in enumerate(
                ...
                if error.get("error_details"):
                    body += (
                        f"   Details: {json.dumps(error['error_details'], indent=2)}\n"
                    )
...
"""
    ...
    def send_hourly_notification(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Sent hourly error notification with {summary['total_errors']} errors"
            )
            return True
    ...
    def cleanup_old_errors(self, days: int = 7):
        ...
        if removed_count > 0:
            self._save_errors()
            logger.info(
                f"Cleaned up {removed_count} old error records (older than {days} days)"
            )
