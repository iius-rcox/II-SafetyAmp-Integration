...
class EmployeeSyncer:
    ...
    def _build_cluster_map(self):
        ...
        cluster_map = {
            ...
            if cluster.get("external_code")
            and cluster.get("parent_cluster_id") is not None
        }
    ...
    def _build_user_map(self):
        ...
        user_map = {
            user["emp_id"]: user for user in users.values() if user.get("emp_id")
        }
    ...
    def map_employee_to_payload(self, emp):
        ...
        return {
            ...
            "roles": (
                [{"id": self.role_map["Field"]}] if "Field" in self.role_map else []
            ),
            "sites": [],
    ...
    def sync(self):
        ...
        for emp in employees:
            ...
            if consecutive_errors >= max_consecutive_errors:
                error_msg = (
                    f"Stopping sync due to {consecutive_errors} consecutive errors"
                )
            ...
            if (
                ...
                and failed_sync_tracker.failed_sync_tracker.should_skip_retry(
                    entity_id=emp_id, entity_type="employee", current_data=payload
                )
            ):
                logger.debug(
                    f"Skipping {full_name} (ID: {emp_id}) - problematic fields unchanged since last failure"
                )
                event_manager.log_skip(
                    "employee", emp_id, "No changes to previously failed fields"
                )
            ...
            if not payload.get("home_site_id"):
                ...
                logger.warning(
                    f"Skipping employee {full_name} (ID: {emp_id}): {reason}"
                )
            ...
            is_valid, validation_errors, cleaned_payload = (
                self.validate_required_fields(payload, emp_id, full_name)
            )
                ...
                if failed_sync_tracker.failed_sync_tracker:
                    failed_sync_tracker.failed_sync_tracker.record_failure(
                        ...
                        error_response={
                            ...
                            "errors": {
                                err.split(":")[0] if ":" in err else "_general": err
            ...
            if existing_user:
                ...
                if updated_fields:
                    ...
                    (
                        ...
                        cleaned_update_payload,
                    ) = self.validate_required_fields(update_payload, emp_id, full_name)
                    ...
                    sanitized_update_fields = {
                        k: cleaned_update_payload.get(k)
                        for k in updated_fields.keys()
                        if k in cleaned_update_payload
                    }
                    ...
                    try:
                        ...
                        self.api_client.patch(
                            f"/api/users/{existing_user['id']}", patch_payload
                        )

                        # Clear failed sync tracker on success
                        if failed_sync_tracker.failed_sync_tracker:
                            failed_sync_tracker.failed_sync_tracker.clear_failure(
                                emp_id, "employee"
                            )

                        logger.info(
                            f"Updated user {full_name} (ID: {emp_id}) with fields: {list(sanitized_update_fields.keys())}"
                        )
                        event_manager.log_update(
                            "employee", emp_id, patch_payload, existing_user
                        )
                        sync_results["updated"] += 1
                        sync_results["processed_employees"].append(
                            {
                                "id": emp_id,
                                "action": "updated",
                                "fields": list(sanitized_update_fields.keys()),
                            }
                        )
                        ...
                        if e.response.status_code == 422:
                            ...
                            logger.error(
                                f"Failed update fields for {full_name}: {updated_fields}"
                            )
                        else:
                ...
                try:
                    ...
                    if e.response.status_code == 422:
                        ...
                        logger.error(
                            f"Failed create payload for {full_name}: {cleaned_payload}"
                        )
                        ...
                        (
                            ...
                            cleaned_fallback_payload,
                        ) = self.validate_required_fields(
                            fallback_payload, emp_id, full_name
                        )
                        ...
                        try:
                            ...
                            event_manager.log_creation(
                                "employee", emp_id, cleaned_fallback_payload
                            )
                            sync_results["created"] += 1
                            sync_results["processed_employees"].append(
                                {"id": emp_id, "action": "created_fallback"}
                            )
        ...
        logger.info(
            f"Employee sync completed: {sync_results['created']} created, {sync_results['updated']} updated, {sync_results['skipped']} skipped, {sync_results['errors']} errors"
        )
