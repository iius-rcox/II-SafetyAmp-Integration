      ...
      <div className="px-6 py-4 border-b border-gray-200">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-semibold text-gray-900">Failed Records Queue</h2>
      ...
      <div className="divide-y divide-gray-200">
        {isLoading ? (
          ...
          data.records.map((record) => {
            ...
            return (
              <div key={recordId} className="hover:bg-gray-50">
                <div
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="font-medium text-gray-900">{record.entity_id}</p>
                        <p className="text-sm text-gray-500">{record.failure_reason}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-4">
                      <div className="text-right text-sm">
                        <p className="text-gray-600">{record.attempt_count} attempts</p>
                        <p className="text-gray-500">Last: {formatRelativeTime(record.last_failed_at)}</p>
                      </div>
                      <div className="flex items-center gap-2">
                        <button
                ...
                {isExpanded && (
                  <div className="px-6 pb-4 bg-gray-50">
                    <div className="space-y-3 text-sm">
                      <div>
                        <span className="font-medium text-gray-700">First Failed:</span>
                        <span className="ml-2 text-gray-600">{formatRelativeTime(record.first_failed_at)}</span>
                      </div>
                      <div>
                        <span className="font-medium text-gray-700">HTTP Status:</span>
                        <span className="ml-2 text-gray-600">{record.http_status}</span>
                      </div>
                      {record.last_error_message && (
                        <div>
                          <span className="font-medium text-gray-700">Error:</span>
                          <p className="mt-1 text-gray-600 bg-white p-2 rounded border border-gray-200 font-mono text-xs">
                            {record.last_error_message}
                          </p>
                        </div>
                      )}
                      {Object.keys(record.failed_fields || {}).length > 0 && (
                        <div>
                          <span className="font-medium text-gray-700">Failed Fields:</span>
                          <div className="mt-1 space-y-1">
                            {Object.entries(record.failed_fields).map(([field, info]) => (
                              <div key={field} className="flex items-center gap-2 text-xs">
                                <span className="font-mono bg-red-100 text-red-800 px-1 rounded">{field}</span>
                                <span className="text-gray-600">{info.error}</span>
                                {info.value && <span className="text-gray-400">({info.value})</span>}
                              </div>
                            ))}
                          </div>
                        </div>
