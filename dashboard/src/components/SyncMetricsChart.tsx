    ...
    <div className="bg-white p-4 rounded-lg shadow-lg border border-gray-200 max-w-sm">
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between gap-4">
          ...
          <span className="font-medium">{formatNumber(data.updates)}</span>
        </div>
        <div className="flex items-center justify-between gap-4">
          <span className="text-sm text-red-600">Errors:</span>
          <span className="font-medium">{formatNumber(data.errors)}</span>
        </div>
        <div className="flex items-center justify-between gap-4">
          <span className="text-sm text-gray-600">Skipped:</span>
          <span className="font-medium">{formatNumber(data.skips)}</span>
        </div>
      </div>
      {data.details && (
    ...
    <div className="bg-white rounded-lg shadow">
      ...
      <div className="px-6 py-4 border-b border-gray-200">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-semibold text-gray-900">Sync Metrics</h2>
          </div>
          <div className="flex items-center gap-3">
            {/* Time Range Buttons */}
        ...
        {data && (
          <div className="mt-4 grid grid-cols-4 gap-4">
            <div className="text-center">
              <p className="text-2xl font-bold text-gray-900">{formatNumber(data.total_syncs)}</p>
      ...
      {data?.by_operation && Object.keys(data.by_operation).length > 0 && (
        <div className="px-6 pb-6">
          <h3 className="text-sm font-medium text-gray-700 mb-3">By Operation</h3>
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            {Object.entries(data.by_operation).map(([op, stats]) => (
              <div key={op} className="bg-gray-50 rounded-lg p-3">
                <p className="text-sm font-medium text-gray-900 capitalize">{op}</p>
